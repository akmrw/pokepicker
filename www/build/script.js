  ...
  `),(await R.query("SELECT COUNT(*) as count FROM pokemon")).values[0].count===0&&await R.execute(`
    ...
    `,n.classList.remove("hidden"),n.classList.add("shown");let{initDatabase:t,getDaten:r,getName:a,getEngName:i,getCardIds:s,updateCardIds:f,insertCard:v,getCardById:S,getCardsByIds:T,updatePrice:U,getTrainers:P,insertTrainer:I,getTrainerCardIds:V,getTrainerById:G,getEnergies:Y,insertEnergie:J,getEnergieCardIds:de,getEnergieById:ue}=await Promise.resolve().then(()=>(Et(),wt));H=await t();let ae=await r(),ie=document.querySelector("#kartentabelle tbody");window.cachedCards={},window.cachedPokemonCardsByName={},window.cachedTrainerCardsByType={},window.cachedEnergyCardsByType={};let le=0,ye=ae.values.length;for(let o of ae.values){let u=document.createElement("tr"),d=parseInt(o.dex),g=`
      ...
      `;u.innerHTML=g,ie.appendChild(u),await q(o.dex),le++;let c=Math.min(99,Math.round(le/ye*100)),p=document.getElementById("progressBar"),m=document.getElementById("progressText");p&&(p.style.width=c+"%"),m&&(m.textContent=c+"%")}progressBar.style.width="100%",progressText.textContent="100%",setTimeout(()=>{let o=n.offsetHeight+"px";n.style.height=o,n.offsetHeight,n.style.transition="height 0.5s ease, opacity 0.5s ease",n.style.height="0px",n.style.opacity="0",n.style.overflow="hidden"},100),x(),k(),N(),document.querySelectorAll("#tableToggle button").forEach(o=>o.classList.remove("active")),document.getElementById("showTablePokemon").classList.add("active");function x(){let o=document.querySelectorAll("#kartentabelle tbody tr"),u=0;o.forEach(d=>{d.style.display!=="none"&&u++}),document.getElementById("eintragsAnzahl").textContent=`(${u})`}function k(){let o=0;document.querySelectorAll("#kartentabelle tbody tr").forEach(m=>{if(m.style.display==="none")return;let b=m.querySelectorAll("div[id^='kartenContainer_'] img");o+=b.length});let u=document.getElementById("kartenAnzahl");u&&(u.textContent=`(${o})`);let d=0;document.querySelectorAll("#trainertabelle tbody tr").forEach(m=>{if(m.style.display==="none")return;let b=m.querySelectorAll("div[id$='Container'] img");d+=b.length});let g=document.getElementById("kartenAnzahlTrainer");g&&(g.textContent=`(${d})`);let c=0;document.querySelectorAll("#energietabelle tbody tr").forEach(m=>{if(m.style.display==="none")return;let b=m.querySelectorAll("div[id$='Container'] img");c+=b.length});let p=document.getElementById("kartenAnzahlEnergie");p&&(p.textContent=`(${c})`)}async function q(o){let u=document.getElementById(`kartenContainer_${o}`),d=await s(o);if(!d)return;let g=d.split(";").filter(m=>m.trim()),c=await T(g.map(m=>parseInt(m)));c.sort((m,b)=>(b.avg30||0)-(m.avg30||0));let p=document.createDocumentFragment();for(let m of c){window.cachedCards[m.cardId]=m;let b=document.createElement("img");b.alt=m.cardId,b.style.width="50px",b.style.height="69px",b.style.objectFit="cover",b.src="cardBackside.png";let y=new Image;y.onload=()=>b.src=m.imageLow,y.onerror=()=>console.warn("Fehler beim Laden von:",m.imageLow),y.src=m.imageLow,b.addEventListener("click",()=>O(o,g.indexOf(String(m.id)))),p.appendChild(b)}u.appendChild(p)}async function O(o,u){let d=document.querySelector("#overlay");d.innerHTML='<div id="overlayContent"><div class="loader"></div></div>',d.classList.remove("hidden"),d.classList.add("shown");let g=await s(o);if(!g)return;let c=g.split(";").filter(b=>b.trim()!=="");if(c.length===0){L();return}let p=u;async function m(){let b=c[p];try{let y=await S(b),h="Unbekannt";if(y.addedAt){let A=new Date(y.addedAt),j={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"};h=A.toLocaleString("de-DE",j)}let l="Keine Angabe";y.basic==1&&(l="Basic"),y.reverse==1&&(l="Reverse"),y.holo==1&&(l="Holo");let w=await M(y.cardId);if(w==null){let A=D(y.cardId);w=await M(A)}let E="\u2013";if(w!=null){let A=w.toFixed(2),j="#DEDEDE",F="\u{1FA99}";w>20?(j="#FF4444",F="\u{1F525}"):w>5&&(j="#FFAA00",F="\u{1F4B0}"),E=`<span style="color:${j};">${F} ${A}\u20AC</span>`}d.innerHTML=`
          ...
          `;let C=new Image;C.onload=()=>{document.getElementById("cardImage").src=y.imageHigh},C.onerror=()=>console.warn("Fehler beim Laden des Galerie-Bildes:",y.imageHigh),C.src=y.imageHigh,document.getElementById("deleteCard").addEventListener("click",async()=>{if(confirm("Willst du diese Karte wirklich l\xF6schen?")){let A=c[p];await H.run("DELETE FROM cards WHERE id = ?",[A]),c.splice(p,1),await f(o,c.join(";")+";"),L();let j=document.getElementById(`kartenContainer_${o}`);j.innerHTML="",q(o),k(),N()}}),document.getElementById("closeGallery").addEventListener("click",()=>{let A=document.getElementById("overlay");A.classList.add("fade-out"),setTimeout(()=>{A.classList.remove("shown","fade-out"),A.classList.add("hidden"),A.innerHTML=""},300)})}catch(y){console.error("Fehler beim Anzeigen der Karte:",y),L()}}m()}window.filterPokemonCardsByNumber=function(){let o=document.getElementById("nummerSuche").value.trim();document.querySelectorAll(".kartenItem").forEach(u=>{let d=u.dataset.number;u.style.display=d&&d.startsWith(o)?"block":"none"})},window.openOverlay=async function(o){let u=document.querySelector("#overlay");u.innerHTML=`
        <div id="overlayContent">
          <h2>Bitte warten...</h2>
          <p>Karten werden geladen</p>
          <div class="loader"></div>
        </div>
      `,u.classList.remove("hidden"),u.classList.add("shown");try{let d=await a(o),g=await i(o);g=="Nidoran\u2642"&&(g="Nidoran \u2642"),g=="Nidoran\u2640"&&(g="Nidoran \u2640"),g=="Farfetch\u2019d"&&(g="Farfetch'd"),g=="Sirfetch\u2019d"&&(g="Sirfetch'd");let c=window.cachedPokemonCardsByName[g];if(c||(c=await K(g),c&&(window.cachedPokemonCardsByName[g]=c)),!c||c.length===0){u.innerHTML=`
            <div id='overlayContent'>
      ...
      `,o.appendChild(c),await Z(),k()};async function Z(){let o=document.getElementById("supporterContainer"),u=document.getElementById("itemContainer"),d=document.getElementById("stadiumContainer"),g=document.getElementById("toolContainer"),c=await P();if(!c||!Array.isArray(c)){console.error("getTrainers() lieferte keine g\xFCltige Liste:",c);return}c.sort((h,l)=>(l.avg30||0)-(h.avg30||0));let p=document.createDocumentFragment(),m=document.createDocumentFragment(),b=document.createDocumentFragment(),y=document.createDocumentFragment();for(let h of c){window.cachedCards[h.cardId]=h;let l=document.createElement("img");l.alt=h.cardId,l.style.width="50px",l.style.height="69px",l.style.objectFit="cover",l.src="cardBackside.png";let w=new Image;w.onload=()=>l.src=h.imageLow,w.onerror=()=>console.warn("Fehler beim Laden von:",h.imageLow),w.src=h.imageLow;let E="";if(Array.isArray(h.subTypes))h.subTypes.some(C=>C.toLowerCase().includes("supporter"))?E="supporter":h.subTypes.some(C=>C.toLowerCase().includes("item"))?E="item":h.subTypes.some(C=>C.toLowerCase().includes("stadium"))?E="stadium":h.subTypes.some(C=>C.toLowerCase().includes("tool"))&&(E="tool");else if(typeof h.subTypes=="string"){let C=h.subTypes.toLowerCase();C.includes("supporter")?E="supporter":C.includes("item")?E="item":C.includes("stadium")?E="stadium":C.includes("tool")&&(E="tool")}l.addEventListener("click",()=>_(h.id,E)),E=="supporter"&&p.appendChild(l),E=="item"&&m.appendChild(l),E=="stadium"&&b.appendChild(l),E=="tool"&&y.appendChild(l)}o.appendChild(p),u.appendChild(m),d.appendChild(b),g.appendChild(y)}window.openTrainerOverlay=async function(o){let u=document.querySelector("#overlay");u.innerHTML=`
        <div id="overlayContent">
          <h2>Bitte warten...</h2>
          <p>Karten werden geladen</p>
          <div class="loader"></div>
      ...
      `,u.classList.remove("hidden"),u.classList.add("shown");try{let d=window.cachedEnergyCardsByType[o];if(d||(d=await me(o),d&&(window.cachedEnergyCardsByType[o]=d)),!d||d.length===0){u.innerHTML=`
        ...
        `;let j=new Image;j.onload=()=>{document.getElementById("cardImage").src=l.imageHigh},j.onerror=()=>console.warn("Fehler beim Laden des Galerie-Bildes:",l.imageHigh),j.src=l.imageHigh,document.getElementById("deleteEnergie").addEventListener("click",async()=>{if(confirm("Willst du diese Karte wirklich l\xF6schen?"))try{await H.run("DELETE FROM energy WHERE id = ?",[h]),L();let F=document.getElementById("basisEnergieContainer"),W=document.getElementById("spezialEnergieContainer");F&&(F.innerHTML=""),W&&(W.innerHTML=""),await oe(),await N()}catch(F){console.error("Fehler beim L\xF6schen der Energie-Karte:",F),alert("Beim L\xF6schen ist ein Fehler aufgetreten.")}}),document.getElementById("closeGallery").addEventListener("click",()=>{L()})}b()}window.filterEnergieCardsByNumber=function(){let o=document.getElementById("nummerSuche").value.trim();document.querySelectorAll(".kartenItem").forEach(u=>{let d=u.dataset.number;u.style.display=d&&d.startsWith(o)?"block":"none"})}})()});})();
