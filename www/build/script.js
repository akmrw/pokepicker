  ...
  `),(await M.query("SELECT COUNT(*) as count FROM pokemon")).values[0].count===0&&await M.execute(`
    ...
    `,n.classList.remove("hidden"),n.classList.add("shown");let{initDatabase:t,getDaten:r,getName:a,getEngName:i,getCardIds:s,updateCardIds:y,insertCard:v,getCardById:L,getCardsByIds:T,updatePrice:Z,getTrainers:I,insertTrainer:A,getTrainerCardIds:Q,getTrainerById:J,getEnergies:se,insertEnergie:Y,getEnergieCardIds:ve,getEnergieById:we}=await Promise.resolve().then(()=>(Wt(),Nt));W=await t();let ue=await r(),me=document.querySelector("#kartentabelle tbody");window.cachedCards={},window.cachedPokemonCardsByName={},window.cachedTrainerCardsByType={},window.cachedEnergyCardsByType={};let ye=0,Pe=ue.values.length;for(let o of ue.values){let d=document.createElement("tr"),c=parseInt(o.dex),g=`
      ...
      `,d.classList.remove("hidden"),d.classList.add("shown");try{let c=await a(o),g=await i(o);g=="Nidoran\u2642"&&(g="Nidoran \u2642"),g=="Nidoran\u2640"&&(g="Nidoran \u2640"),g=="Farfetch\u2019d"&&(g="Farfetch'd"),g=="Sirfetch\u2019d"&&(g="Sirfetch'd");let l=window.cachedPokemonCardsByName[g];if(l||(l=await K(g),l&&(window.cachedPokemonCardsByName[g]=l)),!l||l.length===0){d.innerHTML=`
        ...
        `;for(let u of l){let f=u.avg30,b="\u2013";if(f!=null){let m=f.toFixed(2),w="#DEDEDE",E="\u{1FA99}";f>20?(w="#FF4444",E="\u{1F525}"):f>5&&(w="#FFAA00",E="\u{1F4B0}"),b=`<span style="color:${w}; font-size:14px;">${E} ${m}\u20AC</span>`}let h=u.id.split("-")[1];p+=`
        ...
        `,d.innerHTML=p,document.getElementById("BackBtn").addEventListener("click",u=>{u.preventDefault(),P()})}catch(c){console.error(c),d.innerHTML=`
        <div id='overlayContent'>
          <h2>Fehler beim Laden.</h2>
          <button class="overlayMenuBtn" id="BackBtn">Schlie\xDFen</button>
        </div>`,document.getElementById("BackBtn").addEventListener("click",g=>{g.preventDefault(),P()})}};function H(o){return o.replace(/-(0*)(\d+)/,(d,c,g)=>"-"+parseInt(g,10))}async function K(o){var d,c;try{let g=`https://api.pokemontcg.io/v2/cards?q=name:"${o}"&orderBy=-set.releaseDate`,l=await de.get({url:g,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(l.status!==200)return console.error("Fehler beim Abrufen der Karten:",l.status,l),null;let p=l.data.data;for(let u of p){let f=(d=u.cardmarket)==null?void 0:d.prices;u.avg30=(c=f==null?void 0:f.avg30)!=null?c:null}return p}catch(g){return console.error("Fehler beim Abrufen \xFCber HTTP:",g.message),null}}async function j(o){var d,c,g,l,p;try{let u=`https://api.pokemontcg.io/v2/cards?q=id:${o}`,f=await de.get({url:u,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(f.status!==200||!((c=(d=f.data)==null?void 0:d.data)!=null&&c.length))return console.error("Fehler beim Abrufen der Karte:",f.status,f),null;let b=(l=(g=f.data.data[0])==null?void 0:g.cardmarket)==null?void 0:l.prices;return(p=b==null?void 0:b.avg30)!=null?p:null}catch(u){return console.error("Fehler beim Preisabruf:",u.message),null}}window.karteAusw\u00E4hlen=async function(o,d,c){var b,h;let g=document.querySelector("#overlay");g.innerHTML='<div id="overlayContent"><div class="loader"></div></div>',g.classList.remove("hidden"),g.classList.add("shown");let l=await de.get({url:`https://api.pokemontcg.io/v2/cards/${o}`,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(l.status!==200){console.error("Fehler beim Abrufen der Karte:",l.status,l);return}let p=l.data.data;p.imageSmall=((b=p.images)==null?void 0:b.small)||null,p.imageLarge=((h=p.images)==null?void 0:h.large)||null;let u=await v(p);if(!u){console.error("Keine ID von insertCard erhalten \u2013 Abbruch.");return}p.id=u,window.cachedCards[p.cardId]=p,g.innerHTML=`
        <div id="overlayContent">
      ...
      `,o.appendChild(c);let g=document.createElement("tr");g.innerHTML=`
        <td>Stadion</td>
        <td>
      ...
      `,o.appendChild(l),await ne(),C()};async function ne(){let o=document.getElementById("supporterContainer"),d=document.getElementById("itemContainer"),c=document.getElementById("stadiumContainer"),g=document.getElementById("toolContainer"),l=await I();if(!l||!Array.isArray(l)){console.error("getTrainers() lieferte keine g\xFCltige Liste:",l);return}l.sort((h,m)=>(m.avg30||0)-(h.avg30||0));let p=document.createDocumentFragment(),u=document.createDocumentFragment(),f=document.createDocumentFragment(),b=document.createDocumentFragment();for(let h of l){window.cachedCards[h.cardId]=h;let m=document.createElement("img");m.alt=h.cardId,m.style.width="50px",m.style.height="69px",m.style.objectFit="cover",m.src="cardBackside.png";let w=new Image;w.onload=()=>m.src=h.imageLow,w.onerror=()=>console.warn("Fehler beim Laden von:",h.imageLow),w.src=h.imageLow;let E="";if(Array.isArray(h.subTypes))h.subTypes.some(k=>k.toLowerCase().includes("supporter"))?E="supporter":h.subTypes.some(k=>k.toLowerCase().includes("item"))?E="item":h.subTypes.some(k=>k.toLowerCase().includes("stadium"))?E="stadium":h.subTypes.some(k=>k.toLowerCase().includes("tool"))&&(E="tool");else if(typeof h.subTypes=="string"){let k=h.subTypes.toLowerCase();k.includes("supporter")?E="supporter":k.includes("item")?E="item":k.includes("stadium")?E="stadium":k.includes("tool")&&(E="tool")}m.addEventListener("click",()=>ke(h.id,E)),E=="supporter"&&p.appendChild(m),E=="item"&&u.appendChild(m),E=="stadium"&&f.appendChild(m),E=="tool"&&b.appendChild(m)}o.appendChild(p),d.appendChild(u),c.appendChild(f),g.appendChild(b)}window.openTrainerOverlay=async function(o){if(!navigator.onLine)return alert("F\xFCr diese Funktion wird eine Internetverbindung ben\xF6tigt!"),null;let d=document.querySelector("#overlay");d.innerHTML=`
        <div id="overlayContent">
          <h2>Bitte warten...</h2>
          <p>Karten werden geladen</p>
          <div class="loader"></div>
      ...
      `,d.classList.remove("hidden"),d.classList.add("shown");try{let c=window.cachedTrainerCardsByType[o];if(c||(c=await _(o),c&&(window.cachedTrainerCardsByType[o]=c)),!c||c.length===0){d.innerHTML=`
          ...
          <div id="overlayContent">
            ...
            <div class="kartenGrid">
        `;for(let l of c){let p=l.avg30,u="\u2013";if(p!=null){let b=p.toFixed(2),h="#DEDEDE",m="\u{1FA99}";p>20?(h="#FF4444",m="\u{1F525}"):p>5&&(h="#FFAA00",m="\u{1F4B0}"),u=`<span style="color:${h}; font-size:14px;">${m} ${b}\u20AC</span>`}let f=l.id.split("-")[1];g+=`
        ...
        `,d.innerHTML=g,document.getElementById("BackBtn").addEventListener("click",()=>{P()})}catch(c){console.error(c),d.innerHTML=`
          <div id='overlayContent'>
            <h2>Fehler beim Laden.</h2>
            <button class="overlayMenuBtn" id="BackBtn">Schlie\xDFen</button>
          </div>`,document.getElementById("BackBtn").addEventListener("click",()=>{P()})}};async function _(o){var p,u,f,b;let d=[],c=1,g=250,l=!0;for(;l;){let h=`https://api.pokemontcg.io/v2/cards?q=supertype:Trainer subtypes:${o}&orderBy=name&page=${c}&pageSize=${g}`,m=await de.get({url:h,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(m.status!==200||!((u=(p=m.data)==null?void 0:p.data)!=null&&u.length)){l=!1;break}let w=m.data.data;for(let E of w){let k=(f=E.cardmarket)==null?void 0:f.prices;E.avg30=(b=k==null?void 0:k.avg30)!=null?b:null}d=d.concat(w),w.length<g?l=!1:c++}return d}window.trainerKarteAusw\u00E4hlen=async function(o){var c,g;let d=document.querySelector("#overlay");d.innerHTML='<div id="overlayContent"><div class="loader"></div></div>',d.classList.remove("hidden"),d.classList.add("shown");try{let l=await de.get({url:`https://api.pokemontcg.io/v2/cards/${o}`,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(l.status!==200){console.error("Fehler beim Abrufen der Karte:",l.status,l);return}let p=l.data.data;p.imageSmall=((c=p.images)==null?void 0:c.small)||null,p.imageLarge=((g=p.images)==null?void 0:g.large)||null;let u=await A(p);if(!u){console.error("Keine ID von insertCard erhalten \u2013 Abbruch.");return}p.id=u,window.cachedCards[p.cardId]=p,d.innerHTML=`
          <div id="overlayContent">
            <h2>Karte hinzugef\xFCgt!</h2>
      ...
      `,o.appendChild(c),await he(),C()};async function he(){var p,u,f;let o=document.getElementById("basisEnergieContainer"),d=document.getElementById("spezialEnergieContainer"),c=await se();if(!c||!Array.isArray(c)){console.error("getEnergies() lieferte keine g\xFCltige Liste:",c);return}c.sort((b,h)=>(h.avg30||0)-(b.avg30||0));let g=document.createDocumentFragment(),l=document.createDocumentFragment();for(let b of c){window.cachedCards[b.cardId]=b;let h=document.createElement("img");h.alt=b.cardId,h.style.width="50px",h.style.height="69px",h.style.objectFit="cover";let m=((p=b.subTypes)==null?void 0:p.toLowerCase())||"";h.addEventListener("click",()=>zt(b.id,m.includes("basic")?"basic":"special")),h.src="cardBackside.png";let w=new Image;w.onload=()=>h.src=b.imageLow,w.onerror=()=>console.warn("Fehler beim Laden von:",b.imageLow),w.src=b.imageLow,(u=b.subTypes)!=null&&u.toLowerCase().includes("basic")?g.appendChild(h):(f=b.subTypes)!=null&&f.toLowerCase().includes("special")&&l.appendChild(h)}o.appendChild(g),d.appendChild(l)}window.openEnergieOverlay=async function(o){if(!navigator.onLine)return alert("F\xFCr diese Funktion wird eine Internetverbindung ben\xF6tigt!"),null;let d=document.querySelector("#overlay");d.innerHTML=`
        <div id="overlayContent">
          <h2>Bitte warten...</h2>
          <p>Karten werden geladen</p>
          <div class="loader"></div>
      ...
      `,d.classList.remove("hidden"),d.classList.add("shown");try{let c=window.cachedEnergyCardsByType[o];if(c||(c=await Gt(o),c&&(window.cachedEnergyCardsByType[o]=c)),!c||c.length===0){d.innerHTML=`
        ...
        `,d.innerHTML=g,document.getElementById("BackBtn").addEventListener("click",()=>{P()})}catch(c){console.error(c),d.innerHTML=`
          <div id='overlayContent'>
            <h2>Fehler beim Laden.</h2>
            <button class="overlayMenuBtn" id="BackBtn">Schlie\xDFen</button>
          </div>`,document.getElementById("BackBtn").addEventListener("click",()=>{P()})}};async function Gt(o){var p,u,f,b;let d=[],c=1,g=250,l=!0;for(;l;){let h=`https://api.pokemontcg.io/v2/cards?q=supertype:Energy subtypes:${o}&orderBy=name&page=${c}&pageSize=${g}`,m=await de.get({url:h,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(m.status!==200||!((u=(p=m.data)==null?void 0:p.data)!=null&&u.length)){l=!1;break}let w=m.data.data;for(let E of w){let k=(f=E.cardmarket)==null?void 0:f.prices;E.avg30=(b=k==null?void 0:k.avg30)!=null?b:null}d=d.concat(w),w.length<g?l=!1:c++}return d}window.energieKarteAusw\u00E4hlen=async function(o){var c,g;let d=document.querySelector("#overlay");d.innerHTML='<div id="overlayContent"><div class="loader"></div></div>',d.classList.remove("hidden"),d.classList.add("shown");try{let l=await de.get({url:`https://api.pokemontcg.io/v2/cards/${o}`,headers:{Accept:"application/json","X-Api-Key":pe.API_KEY,Connection:"close"}});if(l.status!==200){console.error("Fehler beim Abrufen der Karte:",l.status,l);return}let p=l.data.data;p.imageSmall=((c=p.images)==null?void 0:c.small)||null,p.imageLarge=((g=p.images)==null?void 0:g.large)||null;let u=await Y(p);if(!u){console.error("Keine ID von insertCard erhalten \u2013 Abbruch.");return}p.id=u,window.cachedCards[p.cardId]=p,d.innerHTML=`
          <div id="overlayContent">
            <h2>Karte hinzugef\xFCgt!</h2>
            <p>Du hast <strong>${o}</strong> ausgew\xE4hlt.</p>
            <br>
        ...
        `,document.getElementById("closeOverlayConfirm").addEventListener("click",()=>{P()})}};async function zt(o,d){var b;let c=document.querySelector("#overlay");c.innerHTML='<div id="overlayContent"><div class="loader"></div></div>',c.classList.remove("hidden"),c.classList.add("shown");let g=await ve();if(!g||!g.length)return;let l=[],p={};for(let h of g){let m=await we(h);if(!m)continue;let w=((b=m.subTypes)==null?void 0:b.toLowerCase())||"";(d==="basic"&&w.includes("basic")||d==="special"&&w.includes("special"))&&(l.push(h),p[h]=m)}if(!l.length){P();return}let u=l.findIndex(h=>h===o);u===-1&&(u=0);async function f(){let h=l[u],m=p[h]||await we(h);if(!m)return;let w="Unbekannt";m.addedAt&&(w=new Date(m.addedAt).toLocaleString("de-DE",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}));let E="Keine Angabe";m.basic==1&&(E="Basic"),m.reverse==1&&(E="Reverse"),m.holo==1&&(E="Holo");let k=m.avg30,D="\u2013";if(k!=null){let $=k.toFixed(2),U="#DEDEDE",ee="\u{1FA99}";k>20?(U="#FF4444",ee="\u{1F525}"):k>5&&(U="#FFAA00",ee="\u{1F4B0}"),D=`<span style="color:${U};">${ee} ${$}\u20AC</span>`}else D='<span style="color:#888;">\u{1F578} Kein Preis gespeichert</span>';c.innerHTML=`
          <div id="overlayContent">
            <button class="closeGallery" id="closeGallery">X</button>
            <h2>Energie</h2>
            <div style="display:flex; align-items:center; justify-content:center;">
