...
document.addEventListener("DOMContentLoaded", () => {
    (async () => {
        ...
        setTimeout(() => {
            ...
            cardLoader.style.height = cardLoader.offsetHeight + "px";
            ...
            async function showCard() {
                ...
                try {
                    ...
                    if (card.addedAt) {
                        ...
                        hinzugefuegtAm = datum.toLocaleString('de-DE', options);
                    }
                          ...
                          <div style="display:flex; align-items:center; justify-content:center;">
                            <div style="display:flex; flex-direction:column; align-items:center;">
                              ...
                              <p style="margin-top:10px;">
                                ...
                                Hinzugefügt am: <strong>${hinzugefuegtAm}
                              </p>
                            </div>
                          </div>
                          <br>
        ...
        window.openOverlay = async function (dex) {
            ...
            try {
                ...
                for (const cardBrief of cards) {
                    ...
                    console.log(cardBrief.image)
                    if (!cardBrief.image || cardBrief.image.indexOf("tcgp") === -1) {

                        //const card = await fetchCardById(cardBrief.id);
                        let preisText = "–";
                        /*const wert30d = card.pricing?.cardmarket?.avg30 || null;
                        ...
                        const nummer = cardBrief.localId;

                        let lowQualityWebp;
                        if (!cardBrief.image) {
                            lowQualityWebp = "cardBackside.png"
                        } else {
                            lowQualityWebp = cardBrief.image + '/low.webp';
                        }

                        html += `
                            <div class="kartenItem" data-id="${cardBrief.id}" data-number="${nummer}" onclick="karteAuswaehlen('${cardBrief.id}', '${dex}', '${name}')">
                              <div>ID: ${cardBrief.id}</div>
                              <img src="${lowQualityWebp}" alt="${name}">
                              <!--<div>${preisText}</div>-->
                            </div>
                          `;
                    }
        ...
        async function fetchCards(engName) {
            const url = `https://api.tcgdex.net/v2/en/cards?name=${encodeURIComponent(engName)}`;
        ...
        window.karteAuswaehlen = async function (cardId, dex, nameFromRow) {
            const overlayElement = document.querySelector("#overlay");
            overlayElement.innerHTML = `<div id="overlayContent"><div class="loader"></div></div>`;
            overlayElement.classList.remove("hidden"); overlayElement.classList.add("shown");

            try {
                let card = await sdk.card.get(cardId);

                const imageLow = card.getImageURL('low', 'webp');
                const imageHigh = card.getImageURL('high', 'webp');

                const cardData = {
                    // WICHTIG: Felder, die insertCard erwartet
                    cardId:    cardId,
                    cardName:  card?.name || nameFromRow || null,
                ...
                // 3) Speichern (deine bestehende Routine)
                const cardDbId = await insertCard(cardData);
                if (!cardDbId) new Error("insertCard lieferte keine ID zurück.");

            } catch (err) {
                console.error("karteAuswählen(TCGdex) Fehler:", err);
                overlayElement.innerHTML = `
                  <div id="overlayContent">
                ...
                document.getElementById("closeOverlayConfirm").addEventListener("click", () => {
                    ...
                    setTimeout(() => { el.classList.remove("shown","fade-out"); el.classList.add("hidden"); el.innerHTML=""; }, 300);
                });
            }
        };
        ...
        async function updateGesamtwert() {
            try {
                const cardResult = await db.query(`SELECT avg30
                                                   FROM cards
                                                   WHERE avg30 IS NOT NULL`);
                const trainerResult = await db.query(`SELECT avg30
                                                      FROM trainer
                                                      WHERE avg30 IS NOT NULL`);
                const energieResult = await db.query(`SELECT avg30
                                                      FROM energy
                                                      WHERE avg30 IS NOT NULL`);
                let summe = 0;
                ...
                const resultCards = await db.query(`SELECT COUNT(*) as count
                                                    FROM cards`);
                const resultTrainer = await db.query(`SELECT COUNT(*) as count
                                                      FROM trainer`);
                const resultEnergie = await db.query(`SELECT COUNT(*) as count
                                                      FROM energy`);
                const totalCards =
        ...
        async function aktualisiereAllePreise() {
            ...
            const result = await db.query(`SELECT cardId
                                           FROM cards`);
            const trainerResult = await db.query(`SELECT cardId
                                                  FROM trainer`);
            const energieResult = await db.query(`SELECT cardId
                                                  FROM energy`);
            ...
            for (const row of trainerResult.values) {
                ...
                if (preis != null) {
                    await db.run(`UPDATE trainer
                                  SET avg30 = ?
                                  WHERE cardId = ?`, [preis, cardId]);
                    aktualisiert++;
                }
      ...
      `;
            ...
            try {
                ...
                for (const card of cards) {
                    ...
                    html += `
            <div class="kartenItem" data-id="${card.id}" data-number="${nummer}" onclick="trainerkarteAuswaehlen('${card.id}')">
              <div>ID: ${card.id}</div>
        ...
        window.trainerkarteAuswaehlen = async function (cardId) {
            const overlayElement = document.querySelector("#overlay");
            ...
            try {
                ...
                console.error("Fehler bei trainerkarteAuswaehlen:", error);
                overlayElement.innerHTML = `
          <div id="overlayContent">
            <h2>Fehler beim Hinzufügen.</h2>
            <p>${error.message}</p>
        ...
        async function openTrainerCardGallery(startId, filterSubType) {
            ...
            async function showTrainerCard() {
                ...
                if (card.addedAt) {
                    const datum = new Date(card.addedAt);
                    hinzugefuegtAm = datum.toLocaleString('de-DE', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    <div style="display:flex; align-items:center; justify-content:center;">
                      <div style="display:flex; flex-direction:column; align-items:center;">
                        ...
                        <p style="margin-top:10px;">
                          ...
                          <br>
                          30d-Wert: <strong>${preisText}</strong><br>
                          Hinzugefügt am: <strong>${hinzugefuegtAm}</strong>
                        </p>
                      </div>
                    </div>
                    <br>
      ...
      `;
        ...
        window.openEnergieOverlay = async function (subType) {
            ...
            try {
                ...
                for (const card of cards) {
                    ...
                    html += `
            <div class="kartenItem" data-id="${card.id}" data-number="${nummer}" onclick="energiekarteAuswaehlen('${card.id}')">
              <div>ID: ${card.id}</div>
              <img src="${card.images.small}" alt="${card.name}">
              <div>${preisText}</div>
            </div>
        ...
        window.energiekarteAuswaehlen = async function (cardId) {
            const overlayElement = document.querySelector("#overlay");
            ...
            try {
                ...
                console.error("Fehler bei energiekarteAuswaehlen:", error);
                overlayElement.innerHTML = `
          <div id="overlayContent">
            <h2>Fehler beim Hinzufügen.</h2>
            <p>${error.message}</p>
